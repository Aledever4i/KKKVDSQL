...
CREATE function [dbo].[CashIncome](@StartDate datetime, @FinishDate datetime, @Cash int)
returns table as 
return select * from
(
select id, cast(null as int ) as id_in_patform, cast (null as int) as PatId, date as [Дата], PaymentCode, summa,
					...
					,Cash,type, Kateg
from CashOrder
where DocType=1/*2*/ and Type=3 and (Cash=@Cash or @Cash is null)
...
where (type=1/* or type=4*/) and pat.id=oa.PatId -- #3182 - только пополнения ЛС
	and (Cash=@Cash or @Cash is null)
) as q 
where /*summa_nal>0 and*/
	[Дата]>=@StartDate and [Дата]<=@FinishDate
