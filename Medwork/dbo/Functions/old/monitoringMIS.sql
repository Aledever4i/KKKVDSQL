CREATE FUNCTION [dbo].[monitoringMIS]
(	
...
(			
	...
	INSERT INTO @Report
			SELECT
				'Количество случаев получения медицинской услуги, зафиксированных в МИС ранее и продолжающихся в текущем отчетном периоде
' as [поле в мониторинге],
(
(			select
				count(*) as [значение]
			FROM
...
' as [поле в мониторинге],
			...
			from 
				forma001 f
			left join TitulniyListF003U t on t.id001=f.id	
			where дата <@ReportDateStart and (DataIVremyaVipiski >=@ReportDateEnd or DataIVremyaVipiski is NULL)
			and дата >'01.02.2017'
