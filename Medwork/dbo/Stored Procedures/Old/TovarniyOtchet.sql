...
create procedure [dbo].[TovarniyOtchet](@StartDate datetime, @FinishDate datetime)
as
...
union all
select 2 as DisplayOrder, '', 'Приход', null, null, null, null
union all
...
select 4 as DisplayOrder, '', 'Приход с остатком', null, null as DocNum, IsNull(SUM(case when h.OperationType in(1,4) then IsNull(p.RetailPrice*p.Quantity*(1.0-isnull(p.DiscountPercent,0.0)/100.0),0) when h.OperationType in (2,3,7) then -(IsNull(pp.RetailPrice*p.Quantity*(1.0-isnull(p.DiscountPercent,0.0)/100.0),0)) end),0), null
	...
	where ((h.OperationType in (1,4,2,3,7) and h.StorageDate<@StartDate)
		...
		and h.Confirmation=1
union all
select 5 as DisplayOrder, '', 'Расход', null, null, null, null
...
null
	...
	where h.OperationType in (1,4,2,3,7)
		...
		and h.Confirmation=1
union all
select 9 as DisplayOrder, '', null, null, null, null, null
union all
...
select 1 as qwe from StorageDocument h
	...
	where h.OperationType in (1,4,2,3,7)
		...
		and h.Confirmation=1
group by h.Provider, h.StorageDate, h.Num
) as qwe), 1)+') документов', null, null, null, null
union all
select 11 as DisplayOrder, '', null, null, null, null, null
union all
select 12 as DisplayOrder, '', 'Отчет с документами принял и проверил', null, null, null, '_________'
union all
select 13 as DisplayOrder, '', null, null, '', null, null
union all
select 14 as DisplayOrder, '', 'Материально-ответственное лицо', null, null, null, '_________'
order by DisplayOrder, DocNum
